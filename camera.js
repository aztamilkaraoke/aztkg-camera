...
async function writeBlobToOpfs(blob, filename) {
  ...
  try {
    ...
    lastSavedOpfsName = filename;
    lastSavedOpfsBlob = blob;
...
function showLastSavedClipLink() {
  if (!els.recentText) return;

  if (!lastSavedOpfsBlob || !lastSavedOpfsName) {
    els.recentText.textContent = '—';
    return;
  }

  const url = URL.createObjectURL(lastSavedOpfsBlob);
  els.recentText.innerHTML =
    '<a id=\"lastClipLink\" href=\"' + url + '\" download=\"' + escapeHtml(lastSavedOpfsName) + '\">Download last clip</a>';
